...
function checkHeader(jsonObj) {
    ...
    if (jsonObj.hasOwnProperty("MsgType") && jsonObj.hasOwnProperty("TTL") && jsonObj.hasOwnProperty("SrcIPorHost") && jsonObj.hasOwnProperty("MsgID")) {
        // check if "MsgType" is "ShareReceipt", "TTL" is number, "SrcIPorHost" is string and "MsgID" is string
        if (typeof jsonObj["MsgType"] === "string" && typeof jsonObj["TTL"] === "number" && typeof jsonObj["SrcIPorHost"] === "string" && typeof jsonObj["MsgID"] === "string") {
            return true;
        }
    }
    return false;
...
// this vast majority function and the accompanying comments were generated by chatGPT, hand written pieces are marked
function checkShareRcptMsg(jsonObj) {
    // check if json object has "Receipt", "TXID" and "SrcPubKey" properties
    if (jsonObj.hasOwnProperty("Receipt") && jsonObj.hasOwnProperty("TXID") && jsonObj.hasOwnProperty("SrcPubKey")) {
        // check if "Receipt" is json, "TXID" is string and "SrcPubKey" is string
        if (typeof jsonObj["Receipt"] === "object" && typeof jsonObj["TXID"] === "string" && typeof jsonObj["SrcPubKey"] === "string") {
            // the next line was written by hand
            return checkRcpt(jsonObj["Receipt"])
        }
    }
    return false;
}
// this entire function and the accompanying comments were generated by chatGPT, except for the if statement noted
function checkReqRcptMsg(jsonObj) {
    // check if json object has "RqstNode" and "Criteria" properties
    if (jsonObj.hasOwnProperty("RqstNode") && jsonObj.hasOwnProperty("Criteria")) {
        // check if "RqstNode" is string and "Criteria" is json
        if (typeof jsonObj["RqstNode"] === "string" && typeof jsonObj["Criteria"] === "object") {
            //the following if statement was added by a human
            if (checkCriteria(jsonObj["Criteria"],acceptableRequestReceiptCriteria)){
                return true;
            }
        }
    }
